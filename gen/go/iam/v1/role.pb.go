...
func (x RoleBindability) MarshalProtoJSON(s *json.MarshalState) {
	s.WriteEnumString(int32(x), RoleBindability_name)
}
	...
	for iNdEx < l {
		...
		case 31:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Bindability |= RoleBindability(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
